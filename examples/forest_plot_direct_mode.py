...
for policy_file in FRESH_DRAWS.glob("*_responses.jsonl"):
    ...
    if len(all_responses) > SAMPLE_SIZE:
        indices = np.random.choice(len(all_responses), SAMPLE_SIZE, replace=False)
        sampled_responses = [all_responses[i] for i in indices]
    else:
        sampled_responses = all_responses
...
# Get oracle truths from diagnostics if available
# In a real scenario, you'd have these from your evaluation set
oracle_truths = {}

# For this example, we'll extract oracle labels from the fresh draws to simulate ground truth
