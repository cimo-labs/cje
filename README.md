...
`calibrated_mean_ci` is the library's bottom layer: a ppi_py-style primitive that takes plain NumPy arrays and returns a calibrated mean and interval. With partial oracle coverage, it defaults to prompt-cluster-robust sampling variance plus the calibration-aware delete-one-oracle-fold jackknife; complete oracle coverage needs no calibrator or calibration jackknife. Request `inference="bootstrap"` for refit-bootstrap percentile intervals. Use it when you have one sample of judge scores and a probability-sampled oracle slice; use `analyze_dataset` for multi-policy comparisons.
...
```text
Calibrated mean: 0.5316 (SE 0.0174, CI [0.4833, 0.5800], n=400, n_oracle=100, cluster_robust)
```
