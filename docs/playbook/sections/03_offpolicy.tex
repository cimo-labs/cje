...
\paragraph{Double robustness:} The estimator is consistent if \emph{either} the stabilized weights $\tilde{w}(\pi')$ converge to the true density ratios (up to mean-one normalization) \emph{or} the outcome model ($\hat{g}$) is correctly specified; if both are good, DR attains the semiparametric efficiency bound.
