...
\begin{itemize}
\item ESS $< 1\%$ $\to$ refuse to estimate (estimate dominated by a handful of points; variance ill-posed).
\item Hill index $\alpha < 2$ $\to$ refuse (weights have infinite variance; estimates unstable and CIs arbitrarily wrong).
\item Coverage $< 50\%$ $\to$ warn strongly (majority of scores extrapolate beyond labeled range; high risk of severe miscalibration).
\end{itemize}
...
\end{equation}
This additivity requires that oracle fold assignments are fixed before analysis begins (no data peeking) and that oracle and evaluation data come from independent sources.
