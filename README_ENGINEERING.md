...
CJE's standard errors account for **two independent sources of variance**:

1. **Main sampling uncertainty** → Cluster-robust for paired designs, standard otherwise
2. **Calibrator uncertainty** (oracle slice) → OUA jackknife

These components are **additive**: `SE_total² = Var_main + Var_OUA`

#### Automatic Cluster-Robust SEs for Paired Comparisons

**Direct Mode only**: When comparing multiple policies on the **same prompts** (paired design), CJE automatically detects prompt alignment and uses cluster-robust standard errors:

```python
# Both files must have matching prompt_ids
result = analyze_dataset(fresh_draws_dir="responses/")

# Check if pairing was detected
if result.metadata.get("prompts_aligned"):
    print("✓ Paired design detected - using cluster-robust SEs")
    print(f"  Clustering by prompt_id ({result.metadata['n_prompts']} unique prompts)")

    # SE method used for each policy
    for policy in result.metadata["target_policies"]:
        se_method = result.metadata["se_methods"].get(policy, "standard")
        n_clusters = result.metadata["n_clusters"].get(policy, "N/A")
        print(f"  {policy}: method={se_method}, n_clusters={n_clusters}")
```

**What happens automatically:**
- CJE checks if all policy files have the same set of `prompt_id` values
- If aligned: uses cluster-robust SE (clusters by `prompt_id`)
- If not aligned: uses standard SE

**Why it matters:** Paired comparisons have correlated outcomes (same prompt → multiple policies). Standard SEs ignore this correlation and understate uncertainty. Clustering by prompt properly accounts for within-prompt dependence.

**Metadata fields:**
- `result.metadata["prompts_aligned"]`: `True` if paired design detected
- `result.metadata["n_prompts"]`: Number of unique prompts (= number of clusters)
- `result.metadata["se_methods"]`: Dict mapping each policy to SE method used (`"cluster_robust"` or `"standard"`)
- `result.metadata["n_clusters"]`: Dict mapping each policy to number of clusters

#### Oracle Uncertainty Aware (OUA) Jackknife

Always enabled when calibration is used. Accounts for uncertainty from learning f(S) on finite oracle slice:
...
if "se_components" in result.metadata:
    ...
    print(f"Includes oracle uncertainty: {components['includes_oracle_uncertainty']}")
```

The total SE you get in `result.standard_errors` already includes both components.
