...
CJE's uncertainty has two sources: **evaluation sampling** (which prompts you drew) and **calibrator uncertainty** (the judge→oracle map was learned from a finite label budget). They may be dependent when calibration and evaluation share prompt clusters. `robust_inference.py` provides the two inference engines `CalibratedDirectEstimator` dispatches between; both propagate both sources on supported calibrated routes.

**1. Cluster-robust SE + oracle jackknife (the default)** — `cluster_robust_se(data, cluster_ids, statistic_fn, ...)` computes CRV1 sandwich SEs; the estimator combines that sampling variance with the delete-one-oracle-fold jackknife variance and uses a t-based CI. The shared OUA recipes live in `cje.diagnostics.robust_inference`: `oracle_jackknife_estimates` (the leave-one-oracle-fold loop), `oracle_jackknife_variance`, and `combine_cluster_and_oracle` (the SE/df combining rule) — one implementation used by both `CalibratedDirectEstimator` and `calibrated_mean_ci`. The two variance components use an approximate Welch–Satterthwaite effective df, so a small oracle component does not force the entire interval to use `K − 1` df. The corrected additive variance procedure is the paper's recommended default; the effective-df rule is separately regression-tested for nominal coverage.

**2. Cluster bootstrap with calibrator refit** — `cluster_bootstrap_direct_with_refit(eval_table, calibrator_factory, n_bootstrap=2000, ...)`. Each replicate draws positive exponential mean-one weights per prompt cluster (a Bayesian cluster bootstrap; the same cluster weights apply to every policy, preserving the paired design — `bootstrap_scheme: "positive_exponential_cluster_weights"` in the returned dict) and refits the calibrator under those weights. This joint refit explicitly resamples calibration/evaluation dependence. Every replicate counts: no replicate is discarded or retried, so the scheme never conditions on "easy" bootstrap worlds. It uses the same augmented estimator and returns percentile CIs plus the `bootstrap_matrix` for paired contrasts. `calibration_policy_idx` restricts calibrator fitting to one policy's labels. When bootstrap is requested but exact calibration provenance is unavailable, the estimator falls back loudly to the analytic jackknife path and records the reason in `result.metadata["inference"]`.
