...
CJE needs at least 10 oracle-labeled samples pooled across policies (2 per calibration fold) — in practice, label 5–25% of your data. The labels can all come from one policy: CJE learns the judge→oracle mapping once and applies it to every policy (the transport audit checks that reuse):
...
- `--estimator`: `calibrated-direct` (default) or `direct`
