...
CJE recommends at least 10 independent oracle-labeled prompt clusters pooled across policies. With 4–9 labeled clusters it reduces the calibration fold count with a warning. Below 4, `analyze_dataset` cannot fit calibration: partially labeled policies return flagged `RAW_JUDGE_MEAN` estimates, while fully labeled policies use `DIRECT_ORACLE_MEAN`. Repeated draws from one prompt do not increase the independent cluster count. Choose the label budget for your sampling design and required precision; no fixed labeling percentage guarantees valid estimates or intervals.

The calibration labels can all come from one policy: CJE learns a shared judge→oracle mapping and applies it to the other policies. Grade that reuse with independent held-out transport probes and a predeclared practical bias margin; without probes the transport state is `NOT_CHECKED`:
...
1. Generate fresh draws per policy, judge everything, and oracle-label a probability sample sized for the required precision.
